package com.sweet.plugin.asm.transformer.clazz
 ...
 * @author johnsonlee
 *
 * 通过 AutoService 注解，booster会将字节码交给当前 Transformer 处理
 */
...
//        this.optimizationEnabled = context.getProperty(PROPERTY_OPTIMIZATION_ENABLED, "$DEFAULT_OPTIMIZATION_ENABLED").toBoolean()
        this.logger =
            context.reportsDir.file("ThreadTransformer").file(context.name).file("report.txt")
                .touch().printWriter()
    }
            ...
            method.instructions?.iterator()?.asIterable()?.forEach {
                ...
                when (it.opcode) {
                    ...
                    Opcodes.ARETURN -> if (method.desc == "L$THREAD;") {
                        //insertBefore 将一个操作码插入到另一个已知的操作码的上面
                        method.instructions.insertBefore(
                            it,
                            LdcInsnNode(makeThreadName(klass.className))
                        )
                        //调用setThreadName
                        method.instructions.insertBefore(
                            it,
...
//com.lanshifu.asm_plugin_library.thread;
